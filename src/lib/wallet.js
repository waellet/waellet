...
export default {
  ...
  async initSdk(cb) {
    ...
    if (typeof keypair.error === 'undefined') {
      const {
        ...
        current,
        subaccounts,
        getActiveAccount: { publicKey },
      } = store.getters;
      const node = await Node({ url: network[current.network].internalUrl, internalUrl: network[current.network].internalUrl });
      const keypairs = await Promise.all(
        subaccounts.map(async (account, activeAccount) => parseFromStorage(await postMessage({ type: 'getKeypair', payload: { activeAccount, account } })))
      );
  ...
  async initContractInstances() {
    ...
    try {
      await store.commit(
        'SET_TOKEN_REGISTRY',
        await store.getters.sdk.getContractInstance(
          store.getters.network[store.getters.current.network].networkId == 'ae_uat' ? TOKEN_REGISTRY_CONTRACT_LIMA : TOKEN_REGISTRY_CONTRACT_LIMA,
          { contractAddress: store.getters.network[store.getters.current.network].tokenRegistry }
        )
      );
    } catch (e) {
      console.log(e);
