      ...
      <div id="account" class="dropdown big" v-if="account.publicKey && isLoggedIn" :slot="menuSlot" direction="center" ref="account">
        ...
        <transition name="slide-fade">
          <ae-list v-if="dropdown.account" class="dropdown-holder">
            <ae-list-item
              fill="neutral"
              @click="changeAccount(index, subaccount)"
              :class="activeAccount == index ? 'activeAccount' : ''"
              v-for="(subaccount, index) in subaccounts"
              v-bind:key="index"
            >
              <ae-identicon class="subAccountIcon" v-bind:address="subaccount.publicKey" size="base" />
              <div class="subAccountInfo">
...
export default {
  ...
  methods: {
    ...
    exportKeypair(type) {
      if (type == 'keypair') {
        const blobData = JSON.stringify({ publicKey: this.account.publicKey, secretKey: this.account.secretKey });
        const blob = new Blob([blobData], { type: 'application/json;charset=utf-8' });
      ...
      this.checkPendingTxInterval = setInterval(() => {
        browser.storage.local.get('pendingTransaction').then(pendingTx => {
          if (
            ...
            (pendingTx.hasOwnProperty('pendingTransaction') && pendingTx.pendingTransaction.hasOwnProperty('list') && Object.keys(pendingTx.pendingTransaction.list).length <= 0)
          ) {
    ...
    async addLedgerAccount() {
      if (this.ledgerNextIdx != 0) {
        const account = await this.$store.dispatch('ledgerCreate');
        if (!account.success) {
    ...
    initLedger() {
      const ledger = new LedgerBridge('https://waellet.com/ledger.html');
