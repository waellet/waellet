  ...
  <div class="popup">
    ...
    <ae-panel>
      <div class="maindiv_input-group-addon">
        <h4>{{ $t('pages.securitySettings.privacyDataHeading') }}</h4>
        <hr />
    ...
    <ae-panel class="decryptKey">
      <div class="maindiv_input-group-addon">
        <h4>{{ $t('pages.securitySettings.privateKeyHeading') }}</h4>
        <hr />
    ...
    <ae-panel>
      <div class="maindiv_input-group-addon">
        <h4>{{ $t('pages.advancedSettings.exportKeystore') }}</h4>
        <hr />
            ...
            if (match) {
              browser.storage.local.get('encryptedSeed').then(async res => {
                if (res.encryptedSeed && res.hasOwnProperty('encryptedSeed')) {
                  ...
                  const decryptedSeed = await decryptMnemonic(res.encryptedSeed, this.password);
