      ...
      <ae-panel>
        ...
        <div v-if="activeTab == 'details'">
          ...
          <div class="flex flex-justify-between tipWebisteAmount">
            <ae-badge :class="selectedTip == index ? 'alternative' : ''" @click.native="selectTip(index)" v-for="(tip, index) in tips" :key="index"
              >{{ tip == 0 ? 'other' : `${tip} ${tokenSymbol}` }}
            </ae-badge>
          </div>
          <div>
...
export default {
  ...
  watch: {
    finalAmount() {
      setTimeout(() => {
        const elem = this.$refs.tipSlider;
    ...
    async fetchTips() {
      if (this.tipping) {
        ...
        this.websiteTips = this.websiteTips
          .map(i => ({ ...i, amount: aettosToAe(i.amount) }))
