      ...
      <transition name="slide">
        <ul class="slideform" :class="dropdown ? 'open' : ''">
          <div class="add-form">
            <ae-input
              class="node-name"
              :label="$t('pages.manageNetworks.networkName')"
              v-model="newUserNetwork"
              :placeholder="$t('pages.manageNetworks.addnetworkName')"
            ></ae-input>
            <ae-input
              class="node-url"
              :label="$t('pages.manageNetworks.networkURL')"
              v-model="newUserNetworkURL"
              :placeholder="$t('pages.manageNetworks.addnetworkURL')"
            ></ae-input>
            <ae-input
              class="node-internalURL"
              :label="$t('pages.manageNetworks.internalURL')"
              v-model="newUserNetworkInternalURL"
              :placeholder="$t('pages.manageNetworks.addinternalURL')"
            ></ae-input>
            ...
            <ae-input
              class="node-middlewareUrl"
              :label="$t('pages.manageNetworks.middlewareUrl')"
              v-model="newUserNetworkMiddlewareURL"
              :placeholder="$t('pages.manageNetworks.addmiddlewareUrl')"
            ></ae-input>
            <ae-input
              class="node-explorerUrl"
              :label="$t('pages.manageNetworks.explorerUrl')"
              v-model="newUserNetworkExplorerURL"
              :placeholder="$t('pages.manageNetworks.addexplorerUrl')"
            ></ae-input>
            <ae-input
              class="node-compilerUrl"
              :label="$t('pages.manageNetworks.compilerUrl')"
              v-model="networkCompilerURL"
              :placeholder="$t('pages.manageNetworks.addcompilerUrl')"
            ></ae-input>
...
export default {
  ...
  methods: {
    removeUserNetwork() {
      const networkName = this.popup.data;
      if (networkName != '') {
        const un = this.userNetworks.filter(d => d.name != networkName);
    ...
    addbtn() {
      ...
      const networkName = this.newUserNetwork;
      const networkURL = this.newUserNetworkURL;
      const networkInternalURL = this.newUserNetworkInternalURL;
      const networkId = this.newNetworkId;
      const networkMiddlewareURL = this.newUserNetworkMiddlewareURL;
      const networkExplorerURL = this.newUserNetworkExplorerURL;
      const { networkCompilerURL } = this;
      const index = this.userNetworks.length - 1;
      const newNetwork = {
        name: networkName,
        url: networkURL,
            ...
            this.$store
              ...
              .then(() => {
                ...
                this.$store.dispatch('switchNetwork', networkName).then(() => {
                  ...
                  const transactions = this.$store.dispatch('getTransactionsByPublicKey', { publicKey: this.account.publicKey, limit: 3 });
...
#manageAccounts .ae-icon-check {
  color: #13b100 !important;
}
#manageAccounts .ae-icon-close {
  color: #b10000 !important;
}
.editaccount:first-child {
...
.editaccount div span,
.editaccount div input,
.addaccount div span,
.editaccount div canvas {
  float: left;
}
.editaccount div button,
.addaccount div button {
  float: right;
}
.editaccount div input {
  width: 60% !important;
