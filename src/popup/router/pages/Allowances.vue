  ...
  <div class="popup">
    ...
    <div v-if="allowancePage == ''">
      ...
      <div>
        <ae-panel>
          ...
          <ae-button class="createAllowance" @click="createAllowancePage" face="round" fill="primary" extend>{{ $t('pages.allowances.createButton') }}</ae-button>
        </ae-panel>
        <ae-panel>
          <h4>{{ $t('pages.allowances.transferAllowance') }}</h4>
          <hr />
          <small class="sett_info">{{ $t('pages.allowances.transferAllowanceInfo') }}</small>
          <ae-button class="transferAllowance" @click="transferAllowancePage" face="round" fill="primary" extend>{{ $t('pages.allowances.transferButton') }}</ae-button>
        </ae-panel>
        <ae-panel>
          <h4>{{ $t('pages.allowances.allAllowances') }}</h4>
          <hr />
          <small class="sett_info">{{ $t('pages.allowances.allAllowancesInfo') }}</small>
          <ae-button class="seeAllAllowance" @click="seeAllAllowancePage" face="round" fill="primary" extend>{{ $t('pages.allowances.seeAllButton') }}</ae-button>
        </ae-panel>
      </div>
    </div>
    <!-- END main page --->
    ...
    <div v-if="allowancePage == 'create'" class="create-allowance allowance-form">
      ...
      <div class="allowanceExistError" v-if="allowanceExistError != '' && allowancePage == 'create' && createform.value != '' && createform.to_account != ''">
        <p>
          ...
          <a class="anchor-here-btn" style="color: #c0c0c0" @click="toChangeAllowanceForm(allowances)">{{ $t('pages.allowances.allowanceExistHere') }}</a>
        </p>
      </div>
    ...
    <div v-if="allowancePage == 'seeAll'" @load="seeAll" class="seeAll-allowance allowance-form">
      ...
      <select class="allowance-token-dropdown" @change="seeAll($event)" v-model="selected">
        <option value="default" disabled>{{ $t('pages.allowances.chooseToken') }}</option>
        <option v-for="(tok, key) in allowanceTokens" v-bind:key="key.id" :value="tok.key">{{ tok.name }}</option>
      </select>
...
import { MAGNITUDE, MIN_SPEND_TX_FEE, MIN_SPEND_TX_FEE_MICRO, FUNGIBLE_TOKEN_CONTRACT } from '../../utils/constants';
  ...
  methods: {
      ...
      Object.entries(this.tokens).forEach(async ([key, val]) => {
        if (val.name != 'AE' && val.contract != '') {
          try {
            ...
            const aex9_extensions = await contract.methods.aex9_extensions();
      ...
      if (this.selected == '' || this.selected == 'default') {
        ...
        this.tokens.forEach(async element => {
          if (element.key == this.selected) {
            try {
              this.loading = true;
              const contract = await this.$helpers.getContractInstance(FUNGIBLE_TOKEN_CONTRACT, { contractAddress: element.contract });
              const checkAmountLeft = await contract.call('allowance', [{ from_account: this.account.publicKey, for_account: this.createform.to_account }], { callStatic: true });
              const amountLeft = await checkAmountLeft.decode().then(async amount => {
                if (amount != 'None') {
                  const tokensLefttoTransfer = element.balance - amount.Some[0];
                ...
                if (this.createform.value > element.balance) {
                  ...
                  try {
                    ...
                    const value = this.createform.value.toString();
                    const create = await this.$helpers.contractCall({ instance: contract, method: 'create_allowance', params: [this.createform.to_account, value] });
                    const dec = await create.decode();
      ...
      if (this.selected == '' || this.selected == 'default') {
        ...
        this.tokens.forEach(async element => {
          try {
            if (element.key == this.selected) {
              this.loading = true;
              const contract = await this.$helpers.getContractInstance(FUNGIBLE_TOKEN_CONTRACT, { contractAddress: element.contract, callStatic: true });
              const checkAmountLeft = await contract.call('allowance', [{ from_account: this.transferform.to_account, for_account: this.account.publicKey }], { callStatic: true });
              const amountLeft = await checkAmountLeft.decode();
              try {
    ...
    async seeAll() {
      ...
      this.tokens.forEach(async element => {
        if (element.key == this.selected) {
          const contract = await this.$helpers.getContractInstance(FUNGIBLE_TOKEN_CONTRACT, { contractAddress: element.contract, callStatic: true });
          const checkAllAllowances = await this.$helpers.contractCall({ instance: contract, method: 'allowances' });
          const all = await checkAllAllowances.decode();
          if (all.length != 0) {
            all.forEach(async singleAllowance => {
              if (singleAllowance[0].for_account == this.account.publicKey) {
                const contract = await this.$helpers.getContractInstance(FUNGIBLE_TOKEN_CONTRACT, { contractAddress: element.contract, callStatic: true });
                const checkAmountLeft = await contract
                  .call('allowance', [{ from_account: singleAllowance[0].from_account, for_account: this.account.publicKey }], { callStatic: true })
                  .then(async amount => {
                    const amountLeft = await amount.decode();
                    const tokensLefttoTransfer = amountLeft.Some[0];
                    this.allowances.push({
                      allowance: element.contract,
    ...
    async changeAllowanceFormBtn() {
      ...
      if (this.changeform.value == '' && this.changeform.value.toString().length == 0) {
        ...
        try {
          const contractId = this.allowances[0].allowance;
          const value = this.changeform.value.toString();
          const contract = await this.$helpers.getContractInstance(FUNGIBLE_TOKEN_CONTRACT, { contractAddress: contractId, callStatic: true });
          const checkAmountLeft = await contract.call('allowance', [{ from_account: this.account.publicKey, for_account: this.changeform.to_account }], { callStatic: true });
          const amountLeft = await checkAmountLeft.decode();
          const tokensLefttoTransfer = amountLeft.Some[0];
          if (value < 0) {
            this.$store.dispatch('popupAlert', { name: 'account', type: 'invalid_number' });
            ...
            const change = await this.$helpers.contractCall({ instance: contract, method: 'change_allowance', params: [this.changeform.to_account, value] });
            const changeDec = await change.decode();
