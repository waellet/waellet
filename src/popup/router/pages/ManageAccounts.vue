...
export default {
  ...
  methods: {
    ...
    nameSave(index) {
      const account = this.accounts[index];
      if (account.name != '') {
        const editedAccounts = this.accounts.map(a => {
          const { edit, ...acc } = a;
          return acc;
        });
    ...
    async addbtn() {
      if (this.newSubAcc != '') {
        const idx = this.subaccounts.filter(s => !s.isLedger && !s.isAirGapAcc).length;
        const address = await this.$store.dispatch('getAccount', { idx });
        this.$store
