<template>
  <div class="popup">
    ...
    <ae-panel>
      ...
      <ae-button face="round" fill="primary" @click="nextStep" v-if="step <= 2" :class="disabledButton ? 'disabled' : ''" extend>{{
        $t('pages.tokenMigration.continue')
      }}</ae-button>
    </ae-panel>
    <popup />
  </div>
...
export default {
  ...
  computed: {
    disabledButton() {
      ...
      if (this.step == 2 && (!this.ethPrivateKey || this.ethPrivateKey == '' || (this.customBackendService && this.backendServiceUrl == ''))) {
        return true;
      }
  ...
  methods: {
    ...
    async nextStep() {
      ...
      if (this.step == 2) {
        const { signature } = this.web3.eth.accounts.sign(this.address, this.ethPrivateKey);
        const { address } = this.web3.eth.accounts.privateKeyToAccount(this.ethPrivateKey);

        const url = this.customBackendService ? this.backendServiceUrl : MIGRATION_SERVICE_URL;
