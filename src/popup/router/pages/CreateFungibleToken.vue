    ...
    <ae-panel>
      ...
      <div>
        ...
        <div v-if="showPrecision" class="input-container token-precision-holder">
          <ae-input :label="$t('pages.createFungibleToken.tokenPrecision')">
            <input
              ...
              v-model.number="token.precision"
              slot-scope="{ context }"
        ...
        <div style="border-radius:4px" class="input-container token-action-holder">
          ...
          <div
            v-if="err.swappableWithoutBurnable"
            style="background: rgb(224, 224, 224);width: 100%;text-align: left;font-size: 0.6875rem;text-transform: uppercase;padding: 10px 1rem;color: #ff0d6a;font-family: 'Inter UI', sans-serif;letter-spacing: 0.2em;font-weight: bold;"
          >
...
export default {
  data() {
    return {
      ...
      tokenRegistry: false,
      showPrecision: false,
      extensions: ['allowances', 'swappable', 'burnable', 'mintable'],
      err: {
        name: false,
        symbol: false,
        precision: false,
  ...
  methods: {
    ...
    confirmTx() {
      ...
      const contractInitArgs = [];
      for (const param in this.token) {
        contractInitArgs.push(this.token[param]);
      }
      const source = aeternityTokens.newToken(this.tokenExtensions);
      const tx = {
        popup: false,
        tx: {
          amount: 0,
