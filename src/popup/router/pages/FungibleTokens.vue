...
export default {
  ...
  methods: {
    ...
    async next() {
      const added = this.tokens.find(tkn => tkn.contract == this.token.contract && tkn.parent == this.account.publicKey);
      if (
        ...
        this.loading = true;
        const call = await this.sdk.contractCallStatic(FUNGIBLE_TOKEN_CONTRACT, this.token.contract, 'balance', [this.account.publicKey]);
    ...
    searchTokenMetaInfo(address) {
      ...
      return new Promise((resolve, reject) => {
        try {
          this.sdk
            .contractCallStatic(FUNGIBLE_TOKEN_CONTRACT, address, 'meta_info')
            .then(res => {
              res
                .decode()
                .then(data => {
