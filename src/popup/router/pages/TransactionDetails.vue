  ...
  <div class="popup">
    ...
    <ae-list class="transactionList ">
      ...
      <ae-list-item fill="neutral" class="flex-direction-column" v-if="isSpendTx">
        <div class="flex-col text-left mb-1 detailTitle">
          {{ $t('pages.transactionDetails.txFrom') }}
          <button :class="transactionType.fill" v-clipboard:copy="transaction.tx.sender_id" @click="copy" class="copyBtn">{{ $t('pages.transactionDetails.copy') }}</button>
        </div>
        <input disabled :value="transaction.tx.sender_id" length="flat" class="transationFrom transactionDetailsInputs" />
      </ae-list-item>
      <ae-list-item fill="neutral" class="flex-direction-column" v-if="isSpendTx">
        <div class="flex-col text-left mb-1 detailTitle">
          {{ $t('pages.transactionDetails.txTo') }}
          <button :class="transactionType.fill" v-clipboard:copy="transaction.tx.recipient_id" @click="copy" class="copyBtn">{{ $t('pages.transactionDetails.copy') }}</button>
        </div>
      ...
      <ae-list-item fill="neutral" class="flex-direction-column" v-if="!isSpendTx && transactionType.fill != ''">
        <div class="flex-col text-left mb-1 detailTitle">
          {{ $t('pages.transactionDetails.txAccount') }}
          <button :class="transactionType.fill" v-clipboard:copy="transaction.tx.account_id" @click="copy" class="copyBtn">{{ $t('pages.transactionDetails.copy') }}</button>
        </div>
      ...
      <ae-list-item fill="neutral" class="flex-direction-column" v-if="isContractCallTx">
        <div class="flex-col text-left mb-1 detailTitle">
          {{ $t('pages.transactionDetails.contractId') }}
          <button :class="transactionType.fill" v-clipboard:copy="transaction.tx.contract_id" @click="copy" class="copyBtn">COPY</button>
        </div>
      ...
      <ae-list-item fill="neutral" class="flex-direction-column" v-if="isNameClaimTx">
        <div class="flex-col text-left mb-1 detailTitle">
          {{ $t('pages.transactionDetails.txName') }}
          <button :class="transactionType.fill" v-clipboard:copy="transaction.tx.name" @click="copy" class="copyBtn">{{ $t('pages.transactionDetails.copy') }}</button>
        </div>
        <div class="flex-justify-items-left transactionName text-left">{{ transaction.tx.name }}</div>
      </ae-list-item>
      <ae-list-item fill="neutral" class="flex-direction-column">
        <div class="flex-col text-left mb-1 detailTitle">
          {{ $t('pages.transactionDetails.txHash') }}
          <button :class="transactionType.fill" v-clipboard:copy="transaction.hash" @click="copy" class="copyBtn">{{ $t('pages.transactionDetails.copy') }}</button>
        </div>
    ...
    <ae-button-group class="btnFixed">
      <ae-button face="round" class=" transactionExplorerBtn" :fill="transactionType.fill != '' ? transactionType.fill : null" @click="transactionInExplorer">
...
export default {
  ...
  computed: {
    ...
    transactionType() {
      ...
      if (this.transaction.tx.type == 'ContractCreateTx') {
        return { fill: 'secondary', type: 'Contract Create Tx' };
      }
      if (this.transaction.tx.type == 'NamePreclaimTx' || this.transaction.tx.type == 'NameUpdateTx' || this.transaction.tx.type == 'NameClaimTx') {
        return { fill: '', type: this.transaction.tx.type };
      }
      if (this.transaction.tx.type == 'ContractCallTx') {
