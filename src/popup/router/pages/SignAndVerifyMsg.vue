<template>
  <div class="popup">
    ...
    <ae-modal-light class="signature-modal" v-if="requirePass" @close="requirePass = false" title="">
      ...
      <ae-input class="my-2" :label="$t('pages.signAndVerifyMsg.password')">
        <input
          type="password"
          class="ae-input"
          :placeholder="$t('pages.signAndVerifyMsg.enterPassword')"
          v-model="password"
          slot-scope="{ context }"
      ...
      <ae-panel>
        ...
        <ae-button
          ...
          @click="
            ...
            verifyMessage = '';
          "
          >{{ $t('pages.signAndVerifyMsg.verifyBtn') }}</ae-button
        >
      </ae-panel>
    </div>
    <div v-if="page == 'sign'">
      ...
      <ae-button face="round" fill="primary" extend :class="[signMessage.length > 0 ? '' : 'disabled']" @click="signMessageAction">{{
        $t('pages.signAndVerifyMsg.signBtn')
      }}</ae-button>
    </div>
    <div v-if="page == 'verify'">
      ...
      <ae-button face="round" fill="primary" extend :class="[verifyMessage.length > 0 ? '' : 'disabled']" @click="verifyMessageAction">{{
        $t('pages.signAndVerifyMsg.verifyBtn')
      }}</ae-button>
    </div>
...
export default {
  ...
  methods: {
      ...
      if (this.password != '') {
        browser.storage.local.get('userAccount').then(async user => {
          if (user.userAccount && user.hasOwnProperty('userAccount')) {
            ...
            const match = await addressGenerator.decryptKeystore(encryptedPrivateKey, this.password);
            this.loading = false;
            if (match) {
    ...
    async signMessageAction() {
      this.loading = true;
      const { secretKey } = await this.$store.dispatch('getKeyPair', { idx: this.activeAccount });
      try {
    ...
    verifyMessageAction() {
      try {
        const verObj = JSON.parse(this.verifyMessage);
        const signature = new Uint8Array(Object.values(verObj.sig));
        const publicKey = Buffer.from(Crypto.decodeBase58Check(this.account.publicKey.split('_')[1]));
