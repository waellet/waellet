...
export default {
  ...
  methods: {
    ...
    async onDecode(content) {
      ...
      const transaction = await this.sdk.sendTransaction(signed);
      const amount = transaction.tx.amount / 1000000000000000000;
      const txUrl = `${this.network[this.current.network].explorerUrl}/transactions/${transaction.hash}`;
      const msg = `You have sent ${amount} AE`;
