<template>
  <div class="popup">
    ...
    <h2>
      <span class="primary">{{ data.params.title }}</span> {{ $t('pages.connectConfirm.websiteRequestconnect') }}
    </h2>
...
export default {
  ...
  methods: {
    ...
    connect() {
      setConnectedAepp(this.data.params.hostname, this.account.publicKey).then(() => {
        if (this.data.popup) {
